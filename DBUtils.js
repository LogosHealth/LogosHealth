 ...
 * @public
 */
exports.getAccountIdFromEmail = function getAccountIdFromEmail(email, session, event, callback) {
  //console.log(' DBUtils.getAccountIdFromEmail >>>>>>');
  loadAccountIDFromEmail(email, session, event, callback);
};
...
exports.instantTemperature = function instantTemperature(intent, session, callback) {
	//console.log(' DBUtils.setScriptContext >>>>>>');
	processTemperature(intent, session, callback);
};

exports.instantSymptom = function instantSymptom(intent, session, callback) {
	//console.log(' DBUtils.setScriptContext >>>>>>');
...
function getScriptDetails(questionId, scriptName, slotValue, session, callback, retUser) {
	...
	connection.query(vSQL, function (error, results, fields) {	
		...
		if (error) {
    		...
    		var errResponse = "There is an error with this request.  If it continues, please contact app support and say error Blue Panda 1.  Restarting LogosHealth.  Please say your first name.";
			...
			console.log('DBUtils.getScriptDetails results gound. results', results);
			if (results !== null && results.length > 0) {
				...
				if (retUser && scriptName.indexOf("Profile") != -1) {
    				...
    				var errResponse = "There is an error in processing this request.  If it continues, please contact app support and say error Blue Panda 2.  Restarting LogosHealth.  Please say your first name.";
					helper.callRestart(session.attributes.accountid, errResponse, session, callback);
				}
			}
		}
			...
			if (results !== null && results.length > 0) {
				if (objType == 'exercise') {
					...
					if (!goalMatch && hasGoalName) {
						...
						console.log('Begin Switch within CompareActiveGoalMatch');
...
//MM 6-10-17 Retrieves the min and max script id for current interview - main interview if current minscriptid = 0
//MM 12-12-18 Combined get max and min together
function getMinScriptId(questionId, scriptName, slotValue, session, callback, retUser, qnaObj) {
	...
	if (session.attributes.minScriptId == 0) {
    	vSQL3="SELECT min(uniquestepid) as minval, max(uniquestepid) as maxval FROM logoshealth.script s, logoshealth.question q where s.questionid=q.questionid and scriptname='"+scriptName+"'";
		...
		connection.query(vSQL3, function (error, results, fields) {    	
			if (error) {
				...
				if (results !== null && results.length > 0) {
					session.attributes.minScriptId = results[0].minval;
					session.attributes.maxScriptId = results[0].maxval;
					closeConnection(connection);
...
function getMaxScriptId (questionId, scriptName, slotValue, session, callback, retUser, qnaObj) {
	...
	if (session.attributes.maxScriptId == 0) {
		...
		connection.query(vSQL2, function (error, results, fields) {    	
			if (error) {
            	console.log('DBUtils.getMaxScriptId connection Error. the Error is: ', error);
				closeConnection(connection);
				...
				//console.log('DBUtils.getMaxScriptId connection results gound. results length is : '+results.length);
				if (results !== null && results.length > 0) {
		...
		if(error)  {
            ...
            if (results !== null && results.length > 0) {
				...
				if(vFields !== null && vFields.length > 0){   						
					vFieldsSplit=vFields.split(","); //This will split based on the comma
                    ...
                    if(results[0].insertnewrow == 'Y') {
					 ...
					 if (results[0].onlyonce == 'N' || sessionAttributes.OOChecked == true) {
                      ...
                      for (var i = 0; i < vFieldsSplit.length; i++) {
						  ...
						  var fieldLabel1 = vFieldsSplit[vFieldsSplit.length - 1];
						  ...
						  connection.query(insertStart, insertSQL, function (error, results, fields) {
                          	if (error) {
								...
								//MM 01-08-19 Temporarily disabling deepstream until real time data entry is incorporated into app
							...
							//MM 1-11-19 updated section to handle multiple variable updates within single statement
							if (tblName == sessionAttributes.currentTable) {
								var updateRec;
								if (vFieldsSplit.length == 1) {
									updateRec="Update "+tblName+" Set "+vFields+" ='"+qnaObj.answer+"' Where "+answerKey+"="+sessionAttributes.tableId; //resArr.answerFieldValue;
								} else {
									var updateSegment1 = "Update "+tblName+" Set ";
									var updateSegment2 = "";
									var updateSegment3 = " Where "+answerKey+"="+sessionAttributes.tableId;
									for (var i = 0; i < vFieldsSplit.length; i++) {
										if (i < vFieldsSplit.length - 1) {
											updateSegment2 = updateSegment2 + vFieldsSplit[i] + " = '" + sessionAttributes[vFieldsSplit[i]] + "', "; 
										} else {
											updateSegment2 = updateSegment2 + vFieldsSplit[i] + " = '" + qnaObj.answer+"' ";
											updateRec = updateSegment1 + updateSegment2 + updateSegment3; 
										}
									}
								}
								connection = getLogosConnection();								   
...
function createNewAccountIDFromEmail(vEmail, session, event, callback, connection)
{
    var connection = getLogosConnection();
	...
	if (error) {
			...
			closeConnection(connection);
			loadAccountIDFromEmail(vEmail, session, event, callback); //Semi-Recursive call. New buzzword from VG.
		}
	});
}

//Load Account ID based on Email registered with the Alexa
//MM 11-9-19 Added event object from Alexa...with this I can use the personid to map to a specific profile
//when voice recognition profile has been set in Alexa
//This function will - look for a personid - if it there check the account for a person with that id.
//if not, work through the email to get accountid 
function loadAccountIDFromEmail(email, session, event, callback) {
	console.log("DBUtil.getAccountFromEmail called with param >>>>> "+email);
	console.log("Session Attributes >>>>> ", session.attributes);
	console.log("Session >>>>> ", session);
	var personid = "";
	
	if (event !== undefined && event.context !== undefined && event.context.System !== undefined && 
	  event.context.System.person !== undefined) {
		personid = event.context.System.person.personId;
		console.log("loadAccountIDFromEmail personid ", personid);
	}

	if (personid !== "") {
		getProfileFromPersonID(email, session, personid, callback);
	} else {
		getAccount(email, session, personid, callback);
	}
}

//MM 11-9-19 This function will retrieve a specific Logos Health user profile based on the Alexa voice recognition profile   
function getProfileFromPersonID(email, session, personid, callback) {
	var connection = getLogosConnection();
	...
	var sql = "SELECT * from logoshealth.profile where alexaid = '" + personid + "' and activeflag = 'Y' and accountid = " +
	  "(SELECT accountid FROM logoshealth.Account where email = '"+ email + "')";
	  console.log('Final SQL for getProfileFromPersonID:' + sql);
	  connection.query(sql, function (error, results, fields) {
		if (error) {
            ...
            if (results !== null && results.length > 0) {
                console.log("DBUtil.getProfileFromPersonID Profile ID found as  >>>>>"+results[0].profileid);
			...
			session.attributes = sessionAttributes;
				...
				userName = results[0].logosname;
				accountId = results[0].accountid;

				//MM 1/8/19 - Creating Profile Object to hold important info for later like home location (latitude, longitude);
				var profileObj = {
					'profileid':profileId,
					'userTimezone':results[0].timezone,
					'badAddress':results[0].badaddress,
					'latitude':results[0].latitude,
					'longitude':results[0].latitude
				}
				console.log('Set UserTimezone in getProfileFromPersonID to ' + session.attributes.userTimezone);
				...
				if (profileComplete) {
					if (!session.attributes.isPrimaryProfile){
						getPrimary(userName, profileId, hasProfile, accountId, profileComplete, profileObj, session, callback);
					} else {
						helper.processNameIntent(userName, profileId, hasProfile, profileComplete, profileObj, session, callback);			
					}	
				} else {
					checkIfAccountHasAnyPrimaryProfile(userName, profileId, hasProfile, accountId, profileComplete, profileObj, session, callback);
				}		
			} else {
				closeConnection(connection);
				console.log('No data found for getProfileFromPersonID');				
				getAccount(email, session, personid, callback);
			}
        }
    });
...
//MM 11-9-19 Added getAccountFromEmail functionality as loadAccountIDFromEmail is now a controller function checking 
//the personid from Alexa Voice recognition.  This can also be called as fall through from getProfileFromPersonID
//When this is the case, the personid value will be set to the profile once the profile is established (through the session.personid) 
function getAccount(email, session, personid, callback) {
	//session.attributes.accountEmail = email;
	var connection = getLogosConnection();
	var accountid = "";

	console.log('Get Account - personid ' + personid);
	if (personid !== "") {
        ...
        if (error) {
            ...
            if (results !== null && results.length > 0) {
                for (var res in results) {
                    ...
                    console.log('from getAccount results - session : ', session);
				...
				closeConnection(connection);
				createNewAccountIDFromEmail(email, session, event, callback, connection);
			}
        }
    });
...
function checkIfAccountHasAnyPrimaryProfile(userName, profileId, hasProfile, accountId, profileComplete, profileObj, session, callback){
	var connection = getLogosConnection();
	//check if any profile exists for this account
	var sql = "select * from logoshealth.profile where accountid="+accountId+" and lower(primaryflag) = 'y' and activeflag='Y'";
...
function processTaskExercise (intent, session, callback) {
	...
	if (strName.toLowerCase() == 'i' || strName.toLowerCase() == 'me' || strName.toLowerCase() == 'myself' || strName.toLowerCase() == 'my' || strName.toLowerCase() == "i'm" || strName.toLowerCase() == "mine") {
...
function getProfileIdsByNameForTaskExercise(strNames, intent, session, callback) {
	...
	if (strNames.constructor === Array) {
		for (var i = 0; i < strNames.length; i++) {
			if (strNames[i].toLowerCase() == 'i' || strNames[i].toLowerCase() == 'me' || strNames[i].toLowerCase() == 'myself' || strNames[i].toLowerCase() == 'my' || strNames[i].toLowerCase() == "i'm" || strNames[i].toLowerCase() == "mine") {
	...
	if (strMood == undefined) {
		...
		connection.query(sql, function (error, results, fields) {
			if (error) {
				...
				closeConnection(connection);
				speechOutput = "Thanks!  Your mood of " + strMood + " has been captured.  Main menu.  For a list of options, simply say menu.";
				helper.gotoMainMenu(speechOutput, session, callback);
			}	
		});	
	}
					...
					for (var j = 0; j < results.length; j++) {
						if(j == results.length -1){
							...
							if (callFunction == 'SleepSummary') {
								...
								saveBloodGlucose(intProfileID, strReturn, true, intent, session, callback);							
							} else if (callFunction == 'Temperature') {
								saveTemperature(intProfileID, strReturn, true, intent, session, callback);							
							} else if (callFunction == 'Symptom') {
								saveSymptom(intProfileID, strReturn, true, intent, session, callback);							
							}
						} else {
							intProfileID = results[j].profileid;	
							if (callFunction == 'SleepSummary') {
								...
								saveBloodGlucose(intProfileID, strReturn, false, intent, session, callback);							
							} else if (callFunction == 'Temperature') {
								saveTemperature(intProfileID, strReturn, false, intent, session, callback);							
							} else if (callFunction == 'Symptom') {
								saveSymptom(intProfileID, strReturn, false, intent, session, callback);							
							}
						}
					}
				}
...
//SECTION TEMPERATURE - FOR SINGLE STATEMENT ADD TEMP
//MM 10-19-18 Adding function to temperature - four cases - "I, me" - process current Profile ID, single family member, retrieve proper profile id
function processTemperature (intent, session, callback) {
	var sessionAttributes = session.attributes;
	var strName = intent.slots.Name.value;
	...
	if (strName.toLowerCase() == 'i' || strName.toLowerCase() == 'me' || strName.toLowerCase() == 'myself' || strName.toLowerCase() == 'my' || strName.toLowerCase() == "i'm" || strName.toLowerCase() == "mine") {
		saveTemperature(sessionAttributes.profileid, 'you', true, intent, session, callback);
	} else {
		getProfileIdsByNameForX('Temperature', strName, intent, session, callback);
	}
}	

function saveTemperature(varProfileId, strFor, exitAfter, intent, session, callback) {
	var connection = getLogosConnection();	
	var sessionAttributes = session.attributes;
	var numTemp = null;

	var blnUserSetTemp = false;
	var dtNow =new Date();
	var momentNow = moment(dtNow);
	var dateNow;

	dateNow = momentNow.format('YYYY-MM-DD HH:mm:ss');

	if (intent.slots !== undefined) {
		if (intent.slots.Temperature !== undefined){
			if (intent.slots.Temperature.value !== undefined) {
				numTemp = intent.slots.Temperature.value;
				console.log('User Temperature: ' + numTemp);	
				blnUserSetTemp = true;
			}
		}
	}
	...
	if (!blnUserSetTemp) {
		console.log('No Data'); 
		var errResponse = "I did not understand the temperature value.  Main Menu.  For a list of options, simply say, Menu.";
		helper.gotoMainMenu(errResponse, session, callback);
	} else {
		var sql2 = "insert into  logoshealth.temperature (temperature, dateofmeasure, profileid, createdby, modifiedby) values (" +
		  numTemp + ", '" + dateNow + "', " + varProfileId + ", " + sessionAttributes.profileid + ", " + sessionAttributes.profileid + ")";
		  console.log('SQL Final for saveTemperature ' + sql2);
		  connection.query(sql2, function (error, results, fields) {	
			if (error) {
				console.log('DBUtils.saveTemperature Error. the Error is: ', error);
				closeConnection(connection); //all is done so releasing the resources
				var errResponse = "There is an error in processing this request.  If it continues, please contact app support and say error in saveTemperature.  Restarting LogosHealth.  Main Menu.  For a list of options, simply say, Menu.";
				helper.gotoMainMenu(errResponse, session, callback);
			} else {
				closeConnection(connection); //all is done so releasing the resources
				if(exitAfter){
					sessionAttributes.currentProcessor = 5;
					sessionAttributes.scriptComplete = true;
					var speechOutput = "Thanks!  The temperature record of " + numTemp + " has been saved for "+ strFor + 
					  ".  Main menu.  For a list of options, simply say menu.";
					helper.gotoMainMenu(speechOutput, session, callback);
				}			
			}					
		});	
...
//SECTION SYMPTOM - FOR SINGLE STATEMENT ADD SYMPTOM
//MM 10-23-18 Adding function to temperature - four cases - "I, me" - process current Profile ID, single family member, retrieve proper profile id
function processSymptom (intent, session, callback) {
	...
	if (strName.toLowerCase() == 'i' || strName.toLowerCase() == 'me' || strName.toLowerCase() == 'myself' || strName.toLowerCase() == 'my' || strName.toLowerCase() == "i'm" || strName.toLowerCase() == "mine") {
		saveSymptom(sessionAttributes.profileid, 'you', true, intent, session, callback);
	} else {
		getProfileIdsByNameForX('Symptom', strName, intent, session, callback);
	}
}	

//MM 10-23-18 Save the symptom record - must handle noun/adjective combinations for symptom and body area relationship
function saveSymptom(varProfileId, strFor, exitAfter, intent, session, callback) {
	var connection = getLogosConnection();	
	var sessionAttributes = session.attributes;
	var strSymptom;
	var strBodyArea = null;
	var strFull;	

	var blnSetSymptom = false;
	var blnHasBody = false;
	var dtNow =new Date();
	var momentNow = moment(dtNow);
	var dateNow;
	...
	if (intent.slots !== undefined) {
		if (intent.slots.Symptom !== undefined){
			if (intent.slots.Symptom.value !== undefined) {
				strSymptom = intent.slots.Symptom.value;
				blnSetSymptom = true;
			}
		}
		if (intent.slots.BodyArea !== undefined){
			if (intent.slots.BodyArea.value !== undefined) {
				strBodyArea = intent.slots.BodyArea.value;
				blnHasBody = true;
			}
		}
	}
	...
	if (!blnSetSymptom) {
		console.log('No Data'); 
		var errResponse = "I did not understand the symptom.  Main Menu.  For a list of options, simply say, Menu.";
		helper.gotoMainMenu(errResponse, session, callback);
	} else {
		if (blnHasBody) {
			if (intent.slots.Symptom.resolutions.resolutionsPerAuthority[0].values !== undefined && Number(intent.slots.Symptom.resolutions.resolutionsPerAuthority[0].values[0].value.id) > 0) {
				var symptomDictId = intent.slots.Symptom.resolutions.resolutionsPerAuthority[0].values[0].value.id;
				getSymptomType (symptomDictId, function(err, result) {
					if (err) {
						strFull = strBodyArea + " " + strSymptom;
						var sql2 = "insert into logoshealth.symptom (symptomname, bodyarea, full_symptom, startdate, profileid, createdby, modifiedby) values ('" +
						strSymptom + "', '" + strBodyArea + "', '" + strFull + "', '" + dateNow + "', " + varProfileId + ", " + 
						  sessionAttributes.profileid + ", " + sessionAttributes.profileid + ")";
						console.log('SQL Final for saveSymptom ' + sql2);
						connection.query(sql2, function (error, results, fields) {	
						  if (error) {
							  console.log('DBUtils.saveSymptom Error. the Error is: ', error);
							  closeConnection(connection); //all is done so releasing the resources
							  var errResponse = "There is an error in processing this request.  If it continues, please contact app support and say error in saveSymptom.  Restarting LogosHealth.  Main Menu.  For a list of options, simply say, Menu.";
							  helper.gotoMainMenu(errResponse, session, callback);
						  } else {
							  closeConnection(connection); //all is done so releasing the resources
							  if(exitAfter){
								  ...
								  if (strBodyArea==null) {
									  ...
									  strBodyArea=strBodyArea + " ";
								  }
								  var speechOutput = "Thanks!  The symptom of " +strFull + " has been saved for "+ strFor + 
									".  Main menu.  For a list of options, simply say menu.";
								  helper.gotoMainMenu(speechOutput, session, callback);
							  }			
						  }					
						...
						if (result == 'adjective') {
							strFull = strSymptom + " " + strBodyArea;
						} else {
							strFull = strBodyArea + " " + strSymptom;
						}
						var sql2 = "insert into logoshealth.symptom (symptomname, bodyarea, full_symptom, startdate, profileid, createdby, modifiedby) values ('" +
			...
			var sql2 = "insert into logoshealth.symptom (symptomname, full_symptom, startdate, profileid, createdby, modifiedby) values ('" +
			strSymptom + "', '" + strSymptom + "', '" + dateNow + "', " + varProfileId + ", " + sessionAttributes.profileid + ", " + sessionAttributes.profileid + ")";
			console.log('SQL Final for saveSymptom ' + sql2);
			connection.query(sql2, function (error, results, fields) {	
			  if (error) {
				  console.log('DBUtils.saveSymptom Error. the Error is: ', error);
				  ...
				  if(exitAfter){
					  ...
					  var speechOutput = "Thanks!  The symptom of " +strSymptom + " has been saved for "+ strFor + 
						".  Main menu.  For a list of options, simply say menu.";
					  helper.gotoMainMenu(speechOutput, session, callback);
				  }			
			  }					
...
//MM 10-24-18 Gets the symptom type (noun or adjective) from the dictionary comments and provides it in callback
function getSymptomType (dictid, callback) {
	var connection = getLogosConnection();	
	var strReturn;

	var sql = "select comment from logoshealth.dictionary where dictionaryid = " + dictid;
	console.log('SQL Final for getSymptomType ' + sql);
	connection.query(sql, function (error, results, fields) {	
	  if (error) {
		  console.log('DBUtils.getSymptomType Error. the Error is: ', error);
		  closeConnection(connection); //all is done so releasing the resources
		  callback(error, null);
	  } else {
		  if(results.length > 0 ){
			strReturn = results[0].comment;
			closeConnection(connection); //all is done so releasing the resources
			callback(null, strReturn);
		 } else {
			closeConnection(connection); //all is done so releasing the resources
...
function saveAddDiet(varProfileId, strFor, exitAfter, intent, session, callback) {
	...
	var strFood = intent.slots.Food.value;
	var strMeal = intent.slots.Meal.value;
	var strCalories = intent.slots.Calories.value;		
	var dtDateofMeasure = session.attributes.dateofmeasure;
	...
	if (strCalories !== undefined && strCalories !==null) {
		varRec.calories = strCalories;
	}
...
//MM 01-09-19 We have made the decision to change the behavior of Order a Meal to default to the broadest possible options if the user has not entered them yet
function checkExistingFoodPreferencesMeal (intent, session, callback) {
	...
	var mealConfig = {
		...
		hasValidRange: false,
		hasFoodCategory: false,
		hasDietPref: false,
		hasValidCalories: false,
		hasValidMeals: false
	}
	//MM 01-09-19 The only pre-requisite for Ordering a Meal is having a valid address - if not, return error.
	if (sessionAttributes.profileObj.badAddress == 'Y') {
		var errResponse = "You must enter a valid home address to leverage this feature.  This can be done through the Logos Health Mobile app or by choosing the, Update Address, from the main menu.  Main menu.  For a list of options, simply say, menu";
		helper.processErrResponse(errResponse, 5, session, callback);	
	} else {
		connection.query(sql, function (error, results, fields) {
			if (error) {
				...
				if (results !== null && results.length > 0) {
					...
					mealConfig.hasFoodPref = true;
					if (results[0].hascost == 'N') {				
						console.log('DBUtil.checkExistingFoodPreferencesMeal Alert: hasCost not configured for profile: ' + sessionAttributes.profileid);
					} else {
						mealConfig.hasValidCost = true;
					}
					if (results[0].hasdelivery == 'N') {
						console.log('DBUtil.checkExistingFoodPreferencesMeal Alert: hasDelivery not configured for profile: ' + sessionAttributes.profileid);
					} else {
						mealConfig.hasValidDelivery = true;
					}
					if (results[0].hasrange == 'N') {
						console.log('DBUtil.checkExistingFoodPreferencesMeal Alert: hasDelivery not configured for profile: ' + sessionAttributes.profileid);
					} else {
						mealConfig.hasValidRange = true;
					}					
					sessionAttributes.foodProfileId = session.attributes.profileid;
					sessionAttributes.mealConfig = mealConfig;
					closeConnection(connection); //all is done so releasing the resources
					...
					checkExistingFoodPreferencesAccountMeal (session, callback);
				}
			}
		});		
	}	
...
function checkExistingFoodPreferencesAccountMeal (session, callback) {
	var connection = getLogosConnection();
	var sessionAttributes = session.attributes;
	...
	connection.query(sql, function (error, results, fields) {
        if (error) {
			...
			if (results !== null && results.length > 0) {
				console.log("DBUtil.foodPreferenceCountMeal foodpreferenceid>>>>> "+ results[0].foodpreferenceid);
				sessionAttributes.mealConfig.hasFoodPref = true;
				if (results[0].hascost == 'N') {				
					console.log('DBUtil.checkExistingFoodPreferencesAccountMeal Alert: hasCost not configured for profile: ' + sessionAttributes.profileid);
				} else {
					sessionAttributes.mealConfig.hasValidCost = true;
				}
				if (results[0].hasdelivery == 'N') {
					console.log('DBUtil.checkExistingFoodPreferencesAccountMeal Alert: hasDelivery not configured for profile: ' + sessionAttributes.profileid);
				} else {
					sessionAttributes.mealConfig.hasValidDelivery = true;
				}
				if (results[0].hasrange == 'N') {
					console.log('DBUtil.checkExistingFoodPreferencesAccountMeal Alert: hasDelivery not configured for profile: ' + sessionAttributes.profileid);
				} else {
					sessionAttributes.mealConfig.hasValidRange = true;
				}					
				sessionAttributes.foodProfileId = results[0].profileid;
				closeConnection(connection); //all is done so releasing the resources
				checkExistingFoodCategoriesMeal(session, callback);
			} else {
				//No Food Categories set up for this profile or account - will default to broadest categories
				sessionAttributes.foodProfileId = sessionAttributes.profileid;
				closeConnection(connection); //all is done so releasing the resources
				checkExistingFoodCategoriesMeal(session, callback);		
			}
		}
        ...
        if (error) {
            ...
            if (results !== null && results.length > 0) {
				...
				if (results[0].yescount == 0) {
					console.log('DBUtil.checkExistingFoodCategoriesMeal ALERT: food categories not configured for profile: ' + sessionAttributes.foodProfileId);
					closeConnection(connection); //all is done so releasing the resources
					checkExistingDietPreferencesMeal(session, callback);
				} else {
					closeConnection(connection); //all is done so releasing the resources
					//has configuration update mealConfig Obj
					sessionAttributes.mealConfig.hasFoodCategory = true;
					checkExistingDietPreferencesMeal(session, callback);
				}
            } else {
				closeConnection(connection); //all is done so releasing the resources
	...
	connection.query(sql, function (error, results, fields) {
        if (error) {
            ...
            if (results !== null && results.length > 0) {
				...
				sessionAttributes.mealConfig.hasDietPref = true;
				if (results[0].hascalories == 'N') {
					console.log('DBUtil.checkExistingDietPreferencesMeal Alert: hasCalories not configured for profile: ' + sessionAttributes.profileid);
				} else {
					sessionAttributes.mealConfig.hasValidCalories = true;
				}
				if (results[0].hasmeals == 'N') {
					console.log('DBUtil.checkExistingDietPreferencesMeal Alert: hasMeals not configured for profile: ' + sessionAttributes.profileid);
				} else {
					sessionAttributes.mealConfig.hasValidMeals = true;
				}				
				closeConnection(connection); //all is done so releasing the resources
				...
				closeConnection(connection); //all is done so releasing the resources
				//Diet prefs not set - move forward using default values (delivered in retrieveMeal) 
				if (sessionAttributes.orderRestaurant !==undefined && sessionAttributes.orderRestaurant !== "" ) {
					verifyRestaurant(session, callback);
				} else {
					retrieveMeal(session, callback);					
...
function retrieveMeal (session, callback) {
	...
	console.log('DBUtils.retrieveMeal: mealConfig', sessionAttributes.mealConfig);
	...
	//MM 01-09-19 - If all checks are good, then all prefs have been configured - go with the complete configuration SQL.  Otherwise use the 
	//default builder function to fill in default values based on identified gaps
	if (sessionAttributes.mealConfig.hasFoodPref && sessionAttributes.mealConfig.hasValidCost && sessionAttributes.mealConfig.hasValidDelivery && 
		sessionAttributes.mealConfig.hasValidRange && sessionAttributes.mealConfig.hasFoodCategory && sessionAttributes.mealConfig.hasDietPref &&
		sessionAttributes.mealConfig.hasValidCalories && sessionAttributes.mealConfig.hasValidMeals) {
			sessionAttributes.mealConfig.hasAllConfig = true;
			console.log('Set hasAllConfig to true');
		} else {
		//console.log('Did not change hasAllConfig');
		//console.log('sessionAttributes.mealConfig.hasFoodPref: ' + sessionAttributes.mealConfig.hasFoodPref);
		//console.log('sessionAttributes.mealConfig.hasValidCost: ' + sessionAttributes.mealConfig.hasValidCost);
		//console.log('sessionAttributes.mealConfig.hasValidDelivery: ' + sessionAttributes.mealConfig.hasValidDelivery);
		//console.log('sessionAttributes.mealConfig.hasValidRange: ' + sessionAttributes.mealConfig.hasValidRange);
		//console.log('sessionAttributes.mealConfig.hasFoodCategory: ' + sessionAttributes.mealConfig.hasFoodCategory);
		console.log('sessionAttributes.mealConfig.hasValidCalories: ' + sessionAttributes.mealConfig.hasValidCalories);
		//console.log('sessionAttributes.mealConfig.hasValidMeals: ' + sessionAttributes.mealConfig.hasValidMeals);
	}
	...
	if (sessionAttributes.mealConfig.hasAllConfig) {
		...
		console.log('Calling SQL builder to build OrderAMeal query with default values in gaps');
		sql = BuildOrderMealSQL(sessionAttributes);
	}
	//01-09-19 Add the varying part of the SQL script to a variable for retrieveMealNext
	sessionAttributes.mealConfig.retrieveSQL = sql;	
...
function retrieveMealNext (session, callback) {
	...
	var sessionAttributes = session.attributes;
	var sql = "";
	const functionName = 'retrieveMealNext';
	var foodProfile;
	var objMenuItem;
	var arrMenuItems = [];
	...
	foodProfile = sessionAttributes.foodProfileId;
	sql =  sessionAttributes.mealConfig.retrieveSQL + " "+ strNotIn + " ";	
...
//MM 01-09-19 Creates Order A Meal SQL decomposed and rebuilt with default values where configuration has not been completed
function BuildOrderMealSQL(sessionAttributes) {
	var finalSQL;	
	var sql = "";
	var sqlAttributes;
	var sqlTables;
	var sqlWhere;
	var blnAddedFP = false;

	const functionName = 'BuildOrderMealSQL';
	var foodProfile;
	
	foodProfile = sessionAttributes.foodProfileId;
	sqlAttributes = "select r.restaurantid, r.restaurantname, r.address, r.city, r.phone, mi.menuitemid, mi.name, mi.description, mi.cost, mi.calories, mi.totalfat, " +
	"mi.carbs, mi.protein ";
	sqlTables = "from logoshealth.menuitem mi, logoshealth.restaurant2menuitem r2m, logoshealth.restaurant r, logoshealth.home2restaurant h2r, logoshealth.profile p";
	sqlWhere = "where mi.menuitemid = r2m.menuitemid and r2m.restaurantid = r.restaurantid and r.restaurantid = h2r.restaurantid and h2r.profileid = p.profileid " +
	"and r.active = 'Y' and r.opennow = 'Y' and mi.category in ('entree', 'soup', 'salad') and p.profileid = " + foodProfile + " ";

	if (sessionAttributes.mealConfig.hasValidRange) {
		sqlTables = sqlTables + ", logoshealth.foodpreference fp, logoshealth.dictionary d1";
		blnAddedFP = true; 
		sqlWhere = sqlWhere + "and p.profileid = fp.profileid and fp.activeflag = 'Y' and fp.deliveryrange = d1.dictionaryid and h2r.searchrange = d1.dictionarycode ";
	} else {
		sqlWhere = sqlWhere + "and h2r.searchrange = 10 ";
	}
	if (sessionAttributes.mealConfig.hasValidCost) {
		if (!blnAddedFP) {
			sqlTables = sqlTables + ", logoshealth.foodpreference fp";
			sqlWhere = sqlWhere + "and p.profileid = fp.profileid and fp.activeflag = 'Y' ";
			blnAddedFP = true; 
		}
		sqlTables = sqlTables + ", logoshealth.dictionary d2";
		sqlWhere = sqlWhere + "and fp.maxcost = d2.dictionaryid " + 
		"and (d2.dictionarycode = 'No Limit' or (d2.dictionarycode >= mi.cost)) ";
	}
	if (sessionAttributes.mealConfig.hasValidDelivery) {
		if (!blnAddedFP) {
			sqlTables = sqlTables + ", logoshealth.foodpreference fp";
			sqlWhere = sqlWhere + "and p.profileid = fp.profileid and fp.activeflag = 'Y' ";
			blnAddedFP = true; 
		}
		sqlTables = sqlTables + ", logoshealth.dictionary d3";
		sqlWhere = sqlWhere + "and fp.deliveryoption = d3.dictionaryid " + 
		"and ((r.delivery_option = 'Yes' and (d3.dictionarycode = 'Delivery' or d3.dictionarycode = 'Both')) " + 
		"or (r.delivery_option = 'No' and (d3.dictionarycode = 'Carry Out' or d3.dictionarycode = 'Both'))) ";
	}
	if (sessionAttributes.mealConfig.hasFoodCategory) {
		sqlTables = sqlTables + ", logoshealth.menuitem_foodcategory mifc, logoshealth.foodcategorypreference fcp";
		sqlWhere = sqlWhere + "and p.profileid = fcp.profileid  and mi.menuitemid = mifc.menuitemid " +
		 "and mifc.foodcategoryid in (select foodcategoryid from logoshealth.foodcategorypreference where answervalue = 'Y' and profileid = " + foodProfile + ") ";
	}
	if (sessionAttributes.mealConfig.hasDietPref) {
		sqlTables = sqlTables + ", logoshealth.dietpreference dp"; 
		sqlWhere = sqlWhere + "and p.profileid = dp.profileid and dp.activeflag = 'Y' " + 
		"and (dp.iswhitemeat = 'N' or (dp.iswhitemeat = 'Y' and mi.iswhitemeat = 'Y')) " +
		"and (dp.ischicken = 'N' or (dp.ischicken = 'Y' and mi.ischicken = 'Y')) " +
		"and (dp.isvegetarian = 'N' or (dp.isvegetarian = 'Y' and mi.isvegetarian = 'Y')) " +
		"and (dp.isvegetariannoegg = 'N' or (dp.isvegetariannoegg = 'Y' and mi.isvegetariannoegg = 'Y')) " +
		...
		"and (dp.hasfishallergy = 'N' or (dp.hasfishallergy = 'Y' and mi.nofish = 'Y')) " +
		"and (dp.hasshellfishallergy = 'N' or (dp.hasshellfishallergy = 'Y' and mi.noshellfish = 'Y')) ";
		if (sessionAttributes.mealConfig.hasValidCalories && sessionAttributes.mealConfig.hasValidMeals) {
			sqlWhere = sqlWhere + "and mi.calories < (dp.targetcaloriesvalue * dp.mealsperentreevalue) "; 
		}
	}
	finalSQL = sqlAttributes + sqlTables + " " + sqlWhere;
	console.log('Final SQL from BuildOrderMealSQL for profile: ' + sessionAttributes.foodProfileId + ': ' + finalSQL);
	return finalSQL;
}
...
function verifyRestaurant (session, callback) {
	...
	foodProfile = sessionAttributes.foodProfileId;
	//MM 01-09-19 Added alternate SQL for when searchRange has not been defined in food preferences
	if (sessionAttributes.mealConfig.hasValidRange) {
		sql = "select r.restaurantid, r.restaurantname from " +
		"logoshealth.home2restaurant h2r, logoshealth.restaurant r, logoshealth.profile p, logoshealth.foodpreference fp, logoshealth.dictionary d1 " +
		...
		sql = "select r.restaurantid, r.restaurantname from " +
		"logoshealth.home2restaurant h2r, logoshealth.restaurant r, logoshealth.profile p " +
		"where r.restaurantid = h2r.restaurantid and h2r.profileid = p.profileid and  r.active = 'Y' and r.opennow = 'Y' and h2r.searchrange = 10 " +
		"and p.profileid = " + foodProfile + " and instr(r.restaurantname, '" + sessionAttributes.orderRestaurant + "') > 0 " +
		"group by r.restaurantid, r.restaurantname";
	}
...
function insertAddWeightRecord (forFamilyProfileId, intent, session, callback) {
	...
	intWeight =  intent.slots.Weight.value;
	if (intWeight.length < 4 ) {
		if (/^\d+$/.test(intWeight)) {
			isnum = true;
		} else {
			isnum = false;
		...
		weightParts = intWeight.split(" ");
		intWeight = 0;
		for (var j = 0; j < weightParts.length; j++) {
			if (weightParts[j] == 'hundred') {
				intWeight = intWeight + 100;
			} else if (/^\d+$/.test(weightParts[j])) {
				intWeight = intWeight + Number(weightParts[j]);
			}
		}
		if (intWeight > 0) {
			isnum = true;
  ...
  if (isnum) {
	...
	connection.query(sql, function (error, results, fields) {
        if (error) {
			...
			closeConnection(connection); //all is done so releasing the resources
			var errResponse = "There is an error in processing this request.  If it continues, please contact app support and say error in insertAddWeightRecord.  Main menu.  For a list of options, simply say 'Menu'";
			helper.callRestart(sessionAttributes.accountid, errResponse, session, callback);
        } else {
            ...
            if (intNewInsert > 0) {
				...
				sessionAttributes.currentProcessor = 5;
				sessionAttributes.scriptComplete = true;
				var speechOutput = "Thanks!  The weight record of " + intWeight + " pounds has been added.  Main menu.  For a list of options, simply say menu.";			
				helper.gotoMainMenu(speechOutput, session, callback);
            } else {
				closeConnection(connection); //all is done so releasing the resources
				//Returns an error call back to the user because no food preference record found
	...
	var errResponse = "The value of " + intent.slots.Weight.value + " could not be entered.  Main menu.  For a list of options, simply say 'Menu'";
	helper.processErrResponse(errResponse, 5, session, callback);	
   }	
...
function getPrimary(userName, profileId, hasProfile, accountId, profileComplete, profileObj, session, callback){
	var connection = getLogosConnection();
	//check if any profile exists for this account
	//console.log("DBUtil.getUserProfileByName - Initiating SQL call "+sql);
	var isPrimary = true;
...
//*************MM 11/9/2019  Adding Code to update a Logos profile with the alexaid (personid) from Alexa voice recognition - 
//this will be done only once to link a Logos Profile with a voice profile********************************/
	...
	connection.query("SELECT * FROM logoshealth.profile where logosname = '"+userName+ "' and accountid = "+accountId+" and activeflag = 'Y'", function (error, results, fields) {
        if (error) {
            ...
            if (results !== null && results.length > 0) {
				...
				console.log('Alexa ID from results: ', results[0].alexaid);
				console.log('session.personid: ', session.attributes.personid);
				if ((results[0].alexaid == undefined || results[0].alexaid == null) && session.attributes.personid !== undefined 
				  && session.attributes.personid !== null) {
					sql = "update logoshealth.profile set alexaid = '" + session.attributes.personid + "' where profileid = " + profileId;
					console.log("Final SQL for update alexaid from getUserProfileByName: " + sql);
					connection.query(sql, function (error2, results2, fields2) {
						if (error2) {
							console.log('Error in updating alexaid for profileid ' + profileId, error2);
						} else {
							console.log('Success in updating alexaid for profileid ' + profileId, results2);
						}
						closeConnection(connection);				
						session.attributes.logosname = userName;
						if (profileComplete) {
					...
					console.log('No personid update needed for personid: ', session.personid);
					closeConnection(connection);				
					session.attributes.logosname = userName;
					if (profileComplete) {
						if (!session.attributes.isPrimaryProfile){
